#!/usr/bin/env bash
# prepare_instance.sh — untimed setup before the timed run.
#   args: v1 <benchmark> <instance> <params>
#
# Not timed, which is what makes it the right place to bring up the CORA background server
# (lazily on the first instance, restarting it if a previous run left it dead or wedged),
# so MATLAB + CORA startup never lands in a measurement. The prepare job itself only
# initializes the GPU; the instance's inputs are generated by run_instance.
#
